    ...
    public String show(@RequestParam(value = "maltc", required = false) Integer maltc,
        ...
        if ("ALL".equals(maKhoa) || maKhoa == null || maKhoa.trim().isEmpty()) {
            dsltc = jdbc.queryForList(
                ...
                "RTRIM(LTC.MAGV) AS MAGV, RTRIM(LTC.MAKHOA) AS MAKHOA, LTC.SOSVTOITHIEU, LTC.HUYLOP, " +
                "MH.TENMH, GV.HO + ' ' + GV.TEN AS HOTENGV, " +
        ...
        try {
            if ("add".equals(action)) {
                jdbc.update("EXEC sp_ThemLopTinChi ?, ?, ?, ?, ?, ?, ?, ?",
                        nienkhoa.trim(), hocky, mamh.trim(), nhom, magv.trim(), kh, sosvtoithieu, huylop ? 1 : 0);
                try {
                ...
                jdbc.update("EXEC sp_SuaLopTinChi ?, ?, ?, ?, ?, ?, ?, ?",
                        maltc, nienkhoa.trim(), hocky, mamh.trim(), nhom, magv.trim(), sosvtoithieu, huylop ? 1 : 0);
