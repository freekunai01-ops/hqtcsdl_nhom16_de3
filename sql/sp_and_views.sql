...
BEGIN
    ...
    BEGIN
        RAISERROR(N'Không thể xóa khoa vì đã có lớp thuộc khoa này!', 16, 1);
        RETURN;
    END
    IF EXISTS(SELECT 1 FROM GIANGVIEN WHERE MAKHOA = @MAKHOA)
    BEGIN
        RAISERROR(N'Không thể xóa khoa vì đã có giảng viên thuộc khoa này!', 16, 2);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN
        ...
        RETURN;
    END
    INSERT INTO LOPTINCHI (NIENKHOA, HOCKY, MAMH, NHOM, MAGV, MAKHOA, SOSVTOITHIEU, SOSVTOIDA, HUYLOP, NGAYBATDAU_DK, NGAYKETTHUC_DK, NGAYHETHAN_HUY, LYDOHUY)
    VALUES (@NIENKHOA, @HOCKY, @MAMH, @NHOM, @MAGV, @MAKHOA, @SOSVTOITHIEU, @SOSVTOIDA, @HUYLOP, @NGAYBATDAU_DK, @NGAYKETTHUC_DK, @NGAYHETHAN_HUY, @LYDOHUY);
END
GO
...
BEGIN
    ...
    UPDATE LOPTINCHI
    SET NIENKHOA = @NIENKHOA, HOCKY = @HOCKY, MAMH = @MAMH, NHOM = @NHOM, MAGV = @MAGV, SOSVTOITHIEU = @SOSVTOITHIEU, SOSVTOIDA = @SOSVTOIDA, HUYLOP = @HUYLOP,
        NGAYBATDAU_DK = @NGAYBATDAU_DK, NGAYKETTHUC_DK = @NGAYKETTHUC_DK, NGAYHETHAN_HUY = @NGAYHETHAN_HUY, LYDOHUY = @LYDOHUY
    WHERE MALTC = @MALTC;
END
GO
...
BEGIN
    ...
    DECLARE @soSVDK INT, @soSVMax INT;
    SELECT @soSVDK = COUNT(*) FROM DANGKY WHERE MALTC = @MALTC AND (HUYDANGKY = 0 OR HUYDANGKY IS NULL);
    SELECT @soSVMax = SOSVTOIDA FROM LOPTINCHI WHERE MALTC = @MALTC;

    IF @soSVDK >= @soSVMax
    BEGIN
        RAISERROR(N'Không thể đăng ký vì lớp tín chỉ đã đủ số lượng tối đa (%d)!', 16, 1, @soSVMax);
        RETURN;
    END
